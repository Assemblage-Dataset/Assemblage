...
class TestScraper(unittest.TestCase):
    def test_github_time_to_mysql_time_good(self):
        ...
        self.assertEqual(scraper.github_time_to_mysql_time(input), "2025-11-01 13:38:46")
    ...
    def test_live_get_request_repo_fields_as_expected(self):
        ...
        expected_tree_keys: set = {
            "path",
            "mode",
            "type",
            "sha",
            "url",
        }  # should be present in every entry in the tree
    ...
    def test_live_get_request_search_fields_as_expected(self):
        ...
        if (
            LiveScraperSettingsGithub.git_token == None
        ):  # unauthenticated requests will not have permissions field
            expected_entry_keys.remove("permissions")
        s = scraper.Scraper(LiveScraperSettingsGithub, 0)

        output: Response
        elapsed: float
        output, elapsed = s.data_source.get_request(GITHUB_REPO_URL, payload=payload)
    ...
    def test_process_repo_message_basic(self, MockRequests, MockTime):
        ...
        self.maxDiff = None
        self.assertEqual(
            message.to_json(), helper.scr_doom_messagestr(), "Returned message not as expected"
        )
    ...
    def test_process_repo_message_invalid_data(self, MockRequests, MockTime):
        ...
        s = scraper.Scraper(ScraperSettingsGithub, 0)
        data_invalid_defaultbranch = {
            "url": "https://api.github.com/users/id-Software",
            "default_branch": "foo",
        }
    ...
    @unittest.skip(
        "Trying to think of a way to rewrite w/out live requests (basically covers the condition where rate limit headers not included)"
    )
    ...
    def test_get_request_about_to_rate_limit(self, MockRequests, MockTime):
        ...
        MockRequests.get.side_effect = [fake_ok_about_to_not_be, fake_nevercalled_hopefully]
        s = scraper.Scraper(ScraperSettingsGithub, 0)
    ...
    def test_get_request_but_bad_credentials(self, MockRequests, MockTime):
        ...
        self.assertEqual(
            MockRequests.get.call_count, 2
        )  # assert that minimum necessary requests were sent (2)
    ...
    def test_bundle_repos(self, MockBlockingConnection, MockBlockingChannel):
        ...
        mock_connection, mock_channel = helper.mock_functioning_rabbitmq(
            MockBlockingConnection, MockBlockingChannel
        )
        ...
        example_repo_msg: ScraperDataOutSingle = ScraperDataOutSingle.from_json(
