...
class Zlib:
    ...
    def run(self):
        ...
        if not os.path.isdir(self.clonedir):
            os.system(f"git clone {self.project_git_url} {self.clonedir})")
        repo = Repo(self.clonedir)
        for tag in self.tags:
            repo.git.checkout("v"+tag)        
            self.repo_current_commit_hash = repo.head.commit.hexsha
            random_dir = tag
            out = cmd_with_output(f"mkdir {random_dir}", platform="windows", cwd=self.clonedir)
            logging.info(out[1])
            arch4cmake = "x64"
            if self.arch == "x86":
                arch4cmake = "Win32"
            ...
            out = cmd_with_output(f"msbuild zlib.sln", platform="windows", 
                                cwd=os.path.join(self.clonedir, random_dir))
            logging.info(out[1])
