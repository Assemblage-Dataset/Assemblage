...
from assemblage.consts import BINPATH, PDBPATH, TASK_TIMEOUT_THRESHOLD, BuildStatus, MAX_MQ_SIZE, WORKER_TIMEOUT_THRESHOLD
...
class Builder(BasicWorker):
    ...
    def scan_binaries(self, clone_dir, repo, original_files):
        ...
        if self.platform == 'linux':
            bin_found = {
                f for f in find_elf_bin(clone_dir) if os.path.exists(f)
            }
            ...
            for filename in glob.glob(clone_dir + '**/**', recursive=True):
                if filename not in original_files and os.path.isfile(filename):
                    dest_file = os.path.join(dest, ntpath.basename(filename))
    ...
    def clone(self, repo, clone_dir):
        ...
        if self.platform == "linux":
            ...
            logging.info("Cur pwd %s, clonedir %s", os.getcwd(), clone_dir)
            out, err, exit_code0 = build_method.cmd_with_output(
                f"wget -O {tmp_filename} {zip_url}", 60, self.platform)
            out, err, exit_code1 = build_method.cmd_with_output(
            ...
            if exit_code == 0:
                ...
                return b'Moving err', BuildStatus.FAILED
    ...
    def job_handler(self, ch, method, _props, body):
        ...
        clone_dir = self.get_clone_dir(task)
        clone_msg, clone_status = self.clone(task, clone_dir)
        folders = []
        original_files = []
        ...
        if clone_status == BuildStatus.SUCCESS:
            if self.platform == "linux":
                ...
                dest_binfolder = os.path.join(BINPATH, str(task["task_id"]))
                build_thread = threading.Thread(target=build_method.linux_build, args=(task, clone_dir, dest_binfolder, self.compiler_flag, 600, self.compiler_version))
                build_thread.start()
                while build_thread.is_alive():
                    self.send_msg(repo=task,
                        ...
                        msg="Still building",
                        commit_hexsha="",
                        build_time=(time.time() - before_build_time))
                    if time.time()-before_build_time>WORKER_TIMEOUT_THRESHOLD:
                        break
                    time.sleep(1)
                after_build_time = time.time()
                zip_file = build_method.post_processing_compress(
                    dest_binfolder, task, self.opt_id, 0)
                if os.path.isfile(zip_file):
                    if os.path.getsize(zip_file)<10*1024:
                        logging.info("File size too small %s bytes", os.path.getsize(zip_file))
                        self.send_msg(repo=task,
                            kind='build',
                            url=url,
                    ...
                    try:
                        s3_file = build_method.post_processing_s3(
                            "platform/"+self.platform+f"_{self.compiler_version}/"+str(task["task_id"])+".zip", zip_file, "default")
                    except Exception as e:
                        logging.error("Error posting binary to S3: %s", e)
                    logging.info("Posted binary `%s` to S3.", zip_file)
                build_method.clean([clone_dir, zip_file, dest_binfolder], platform="linux")
                return
            ...
            for library, mode, version, opti in build_task_configs:
                ...
                if build_status == BuildStatus.SUCCESS:
                    ...
                    if self.platform == "windows":
                        ...
                        repo_fname = dest_binfolder.split("/")[-1]
                        file_location = f"{BINPATH}/{zip_file}"
                    if self.send_binary_method.lower() == "ftp":
                        ...
                        try:
                            s3_file = build_method.post_processing_s3(
                                "platform/"+self.platform+"/"+str(task["task_id"])+".zip", file_location)
                            if s3_file:
