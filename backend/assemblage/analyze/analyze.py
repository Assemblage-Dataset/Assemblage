...
def get_build_system(files):
    ...
    build_systems = {"make": ["makefile", "makefile.in", "makefile.am"],
    ...
    priority_order = ["cmake", "make", "scons", "meson", "ninja", "bootstrap", "autoconf", 
                     "sln", "bazel", "buck", "qmake", "premake", "xcodebuild", 
                     "cargo", "gradle", "maven", "python"]
    ...
    if not detected_tools:
        ...
        has_cpp = any(any(f.endswith(ext) for ext in ['.cpp', '.cc', '.cxx', '.c++']) 
                     for f in files)
