...
from assemblage.consts import (
    ...
    BuildStatus,
    CloneStatus,  # REPO_SIZE_THRESHOLD,
    InputQueue,
    OutputQueue,
    ScraperMsgType,
)
...
from assemblage.mq.client import Connection, MessageClient, MQQueue
...
class Coordinator:
    ...
    def __init__(self, settings: CoordinatorSettings):
        ...
        self.mq_client = MessageClient(
            settings.mq_host, settings.mq_port, username="guest", password="guest"
        )
    ...
    def __dispatch_task(
        self, build_opt_id, sleep=True, only_run_once=False
    ):  # last arg is for tests
        ...
        try:
            ...
            self._dispatch_queue_map[build_opt_id] = MQQueue(
                name=f"{OutputQueue.BUILD_OPT}_{build_opt_id}",
                exchange_name=f"{OutputQueue.BUILD_OPT}",
                routing_key=f"{OutputQueue.BUILD_OPT}_{build_opt_id}",
            )
            conn: Connection = self.mq_client.create_connection(
                conn_name=f"{self}-build-opt-{build_opt_id}",
                channel_name=f"{self}-build-opt-{build_opt_id}",
            )
            ...
            logger.info(
                "__dispatch_task started successfully, %s tasks are ready to be queued for dispatch on build_opt_%d",
                num_tasks,
                build_opt_id,
            )
        except:
        ...
        while True:
            try:
                ...
                task_count += self._dispatch_to_builder(build_opt_id, conn, sleep, task_count)
    ...
    def _dispatch_to_builder(self, build_opt_id, conn: Connection, sleep: bool, task_count: int):
        """
        ...
        builder_receive_queue = MQQueue(
            f"{OutputQueue.BUILD_OPT}_{build_opt_id}",
            routing_key=f"{OutputQueue.BUILD_OPT}_{build_opt_id}",
        )
        ...
        if (
            build_message is None
        ):  # no more scraped repos to dispatch. determine whether to idle or request repos
            ...
            if messages_on_buildopt <= COORDINATOR_REPO_REQUEST_THRESHOLD:
                ...
                logger.debug(
                    f"Dispatch thread on build option {build_opt_id} waiting for dispatch request to be sent"
                )
                with self.t_empty_built_opt_lock:
                ...
                logger.info(
                    f"Dispatch thread on build option {build_opt_id} idling ({messages_on_buildopt} tasks waiting to be built)"
                )
            ...
            conn.send_msg(
                queue=self._dispatch_queue_map[build_opt_id],
                msg=build_message.to_json().encode(),
                exchange=f"{OutputQueue.BUILD_OPT}",
            )

            self.db_man.update_repo_status(
                status_id=build_message.task_id, clone_status=CloneStatus.PROCESSING
            )
    ...
    def _request_repos(self, build_opt_id: int, conn: Connection):
        ...
        try:
            ...
            with self._scraper_queues_lock:
                targets = (
                    list(self._scraper_queues)
                    if self._scraper_queues
                    else [OutputQueue.SCRAPER_CTRL]
                )
    ...
    def __recycle_clone(self):
        ...
        while True:
            count = 0
            try:
                for repo in self.db_man.find_repo_by_status(
                    ...
                    for b_status in self.db_man.find_status_by_repoid(repo.id):
                        if (
                            b_status.clone_status == CloneStatus.FAILED
                            and b_status.build_status == BuildStatus.INIT
                        ):
                            self.db_man.update_repo_status(
                                status_id=b_status.id, clone_status=CloneStatus.NOT_STARTED
                            )
                            count += 1
    ...
    def __clean_overtime(self):
        ...
        while True:
            ...
            logger.info(">>>>>>>>>>>>>>>>>>>>>> cleanning overtime tasks ......")
    ...
    def recv_scrape_info(
        ...
        try:
            ...
            if ch and ch.is_open:
                ...
                logger.info(
                    f"Received {len(bundle)} / saved {successes} repos in {round(time.time() - start_time, 2)}s"
                )

            else:
                logger.warning(
                    "Channel recv_scrape_info closed before ack, message will be redelivered"
                )
    ...
    def __run_scraper_ctrl(self):
        ...
        try:
            conn: Connection = self.mq_client.create_connection(
                conn_name=f"{self}-{OutputQueue.SCRAPER_CTRL}",
        ...
        try:
            ...
            if task.clone_status != CloneStatus.SUCCESS:
                ...
                if task.clone_status in [CloneStatus.NOT_STARTED, CloneStatus.PROCESSING]:
                    ...
                    while timeout > 0 and task.clone_status in [
                        ...
                        task = self.db_man.get_status_row_by_id(recv_msg.task_id)
                if (
                    task.clone_status != CloneStatus.SUCCESS
                ):  # sync attempt timed out or clone was a failure
            ...
            logger.debug(
                f"BUILD task {task} on buildopt %s updated to %s: %s",
                recv_msg.opt_id,
                recv_msg.status,
                " ".join(recv_msg.msg.split())[-500:],
            )
            if recv_msg.status in [BuildStatus.SUCCESS, BuildStatus.FAILED]:
                if recv_msg.status == BuildStatus.SUCCESS:
                    ...
                    skipped = self.db_man.fail_sibling_statuses(
                        task.repo_id, recv_msg.task_id, msg="Sibling build failed"
                    )
                    if skipped > 0:
                ...
                if (self.info_successes + self.info_failures) % 100 == 0:
                    logger.info(
                        f"Builds completed: {self.info_successes + self.info_failures} ({self.info_successes} successes, {self.info_failures} failures)"
                    )
    ...
    def recv_clone_info(self, ch, method, _props, body):
        ...
        try:
            ...
            if task.clone_status != BuildStatus.SUCCESS:
                logger.info(
                    f"CLONE task {task} on buildopt %s updated to %s: %s",
                    recv_msg.opt_id,
                    task.clone_status,
                    recv_msg.msg,
                )
    ...
    def recv_builder_registration(self, ch, method, props, body):
        ...
        try:
            ...
            if build_opt_id is None:
                logger.error(
                    "Failed to register build option for builder %s — skipping", reg_info.name
                )
                if ch and ch.is_open:
                    ch.basic_nack(delivery_tag=method.delivery_tag, requeue=True)
                return

            conn: Connection = self.mq_client.get_connection(
                conn_name=f"{self}-{InputQueue.BUILD_REG}"
            )
            if conn:
                conn.ensure_connection()
            else:
                conn: Connection = self.mq_client.create_connection(
                    conn_name=f"{self}-{InputQueue.BUILD_REG}",
                    channel_name=f"{self}-{InputQueue.BUILD_REG}",
                )

            # Respond on the caller-provided reply queue (unique per builder)
            reply_queue_name = (
                props.reply_to if props and props.reply_to else OutputQueue.BUILDER_CTRL
            )
            queue = MQQueue(reply_queue_name)

            conn.send_msg(
                queue=queue,
                msg=BuilderRegOut(build_opt_id).to_json(),
                exchange="",
            ...
            if ch and ch.is_open:
                ...
                logger.warning(
                    "Channel for builder registration closed before ack, message will be redelivered"
                )
    ...
    def recv_scraper_reg(self, ch, method, props, body):
        ...
        try:
            ...
            if request_msg.message_type == ScraperMsgType.SETUP:
                logger.debug(
                    f"Received scraper request for setup info, correlation id {props.correlation_id}"
                )
                ...
                get_config = self.db_man.register_scraper(
                    props.correlation_id, request_msg.start_time, request_msg.end_time
                )
                ...
                msg = ScraperControlTaskOut(
                    message_type=ScraperMsgType.UPDATE, start_time=None, end_time=None
                )

            conn: Connection = self.mq_client.get_connection(
                conn_name=f"{self}-{InputQueue.SCRAPER_REG}"
            )
            if conn:
                conn.ensure_connection()
            else:
                conn = self.mq_client.create_connection(
                    conn_name=f"{self}-{InputQueue.SCRAPER_REG}",
                    channel_name=f"{self}-{InputQueue.SCRAPER_REG}",
                )

            reply_queue_name = (
                props.reply_to if props and props.reply_to else OutputQueue.SCRAPER_CTRL
            )
            queue = MQQueue(reply_queue_name)
    ...
    def run(self):
        ...
        t_consume_clone = threading.Thread(
            ...
            target=self.__consume_from_queue,
            args=(InputQueue.CLONE,),
        )
