...
class LinuxBuildStrategy(BuildStrategy):
    ...
    def run_build(
        ...
        logger.debug(f"Files found in {clone_dir} {os.listdir(clone_dir)}")
    ...
    def post_build_hook(
        self, dest_binfolder, repoinfo, compiler_flag, commit_hexsha, original_files=None
    ):
    ...
    def _collect_functions_from_die(
        self, die, functions_list, base_addr, file_table, dwarf_info, cu_base_addr
    ):
        ...
        if die.tag in ("DW_TAG_subprogram", "DW_TAG_inlined_subroutine"):
            ...
            if func_name:
                addr_ranges = self._resolve_address_ranges(die, dwarf_info, cu_base_addr)
        ...
        for child in die.iter_children():
            self._collect_functions_from_die(
                child, functions_list, base_addr, file_table, dwarf_info, cu_base_addr
            )
    ...
    def _extract_dwarf_info(self, binfile):
        ...
        with open(binfile, "rb") as f:
            ...
            for CU in dwarf_info.iter_CUs():
                ...
                self._collect_functions_from_die(
                    top_die, all_functions, base_addr, file_table, dwarf_info, cu_base_addr
                )
                ...
                if line_program:
                    ...
                    for entry in line_program.get_entries():
                        ...
                        if source_file and os.path.isfile(source_file):
                            if source_file not in file_cache:
                                try:
                                    with open(
                                        source_file, encoding="utf-8", errors="replace"
                                    ) as sf:
                    ...
                    for i in range(len(raw_entries) - 1):
                        raw_entries[i]["length"] = (
                            raw_entries[i + 1]["rva_int"] - raw_entries[i]["rva_int"]
                        )
            ...
            # Build output
            item_dict = {"file": os.path.basename(binfile), "functions": []}
