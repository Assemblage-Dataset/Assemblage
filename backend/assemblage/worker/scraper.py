...
class GithubRepositories(DataSource):
    ...
    def __init__(
        ...
        proxies: list,
        sort=GithubTimeOrder.CREATED,
        build_sys_callback=get_build_system,
        alternate_git_tokens: list[str] | None = None,
    ) -> None:
    ...
    def _process_repo_message(self, repo):
        ...
        if type(repo) is not dict:
            logger.error(f"_process_repo_message expects dictionary as input, not {type(repo)!s}")
            return None, None
        url = repo["url"]
    ...
    def fetch_data(self):
        ...
        while (
            self.current_crawl_time > self.crawl_time_end
        ):  # continue until oldest files have been read
            ...
            payload = {"q": query_s, "per_page": SCRAPER_PAGE_SIZE, "page": -1}
            ...
            while payload["page"] * SCRAPER_PAGE_SIZE < total_query_results_count:
                try:
                    ...
                    if "items" in rdict.keys():
                        ...
                        logger.debug(
                            "Successful search result obtained by crawler %s. GitHub responded with %s repos",
    ...
    def get_request(
        self, query: str, payload: set = None, headers="default", proxy: str = "random"
    ):
        """Gets the requested query, handling rate limits as necessary.
        ...
        if r.status_code == 401:
            logger.error(
                "401 Unauthorized. The authentication token provided is not valid. Please provide a valid token."
            )
    ...
    def cycle_token(self):
        ...
        for t in range(len(self.alternate_tokens)):
            if self.alternate_tokens[t]["timeout"] == 0 or self.alternate_tokens[t][
                "timeout"
            ] <= int(time.time()):
...
class Scraper(BasicWorker):
    ...
    def __init__(self, settings: ScraperSettings, workerid: int):
        ...
        super().__init__(
            settings.name, settings.mq_host, settings.mq_port, worker_type=WorkerType.Scraper
        )
    ...
    def check_for_update(self):
        ...
        if self._last_sent_crawltime != self.data_source.current_crawl_time:
            ...
            logger.debug(
                f"Updating time from {self._last_sent_crawltime} to {self.data_source.current_crawl_time}"
            )
            ...
            conn.send_msg(queue=send_queue, msg=msg.to_json(), corr_id=self.uuid)
    ...
    def send_bundle(self):
        ...
        conn.send_msg(queue=self.scrape_queue, msg=bundle.to_json(), corr_id=self.uuid)
        self.total_repos_sent += len(self.repocache)
        logger.info(
            "Scraper %s bundled and sent %s repos to coordinator. Total repos sent by this scraper: %s",
            self.workerid,
            len(self.repocache),
            self.total_repos_sent,
        )
        self.repocache = []
    ...
    def control_message_handler(self, ch, method, props, body):
        ...
        if isinstance(self.data_source, GithubRepositories):
            ...
            logger.warning(
                f"Error: data source of type {type(self.data_source)} does not have defined behavior for responding to coordinator setup msg"
            )
    ...
    def handle_in_messages_github(self, ch, method, props, msg):
        ...
        if msg.message_type in [ScraperMsgType.SETUP, ScraperMsgType.UPDATE]:
            ...
            debug_starttime = datetime.utcfromtimestamp(self.data_source.crawl_time_start).strftime(
                "%Y-%m-%d-%H"
            )
            debug_endtime = datetime.utcfromtimestamp(self.data_source.crawl_time_end).strftime(
                "%Y-%m-%d-%H"
            )
            self.ready = True
            if msg.message_type == ScraperMsgType.SETUP:
                logger.debug(
                    f"Configured scraper {self.workerid} to start scraping from {debug_starttime} to {debug_endtime} on policy '{self.policy}'"
                )
    ...
    def run_ctrl(self):
        ...
        try:
            while True:
                conn: Connection = self.mq_client.create_connection(
                    conn_name=f"{self}-ctrl", channel_name=f"{self}-ctrl"
                )
                conn.create_channel()
        ...
        try:
            ...
            while not self.ready:
                if waited_time >= next_log_time:
                    logger.info(
                        f"Scraper {self.workerid} waiting for config ({round(waited_time, 2)}s)..."
                    )
                    next_log_time += 10.0
            ...
            for repo in iter(self.data_source):
                ...
                self.repocache.append(
                    repo
                )  # don't put this in any if statements, to avoid losing repos
                ...
                # continually collect repositories until enough are collected, then sleep until bundle requested
                    ...
                    if len(self.repocache) >= SCRAPER_REPO_BUNDLESIZE:
                        ...
                        if len(self.repocache) == SCRAPER_REPO_BUNDLESIZE:
                            logger.info(
                                f"Scraper {self.workerid} has collected max number of repos ({SCRAPER_REPO_BUNDLESIZE}) in {round(time.time() - self._last_bundled_time, 3)}s. Sleeping until request to send is received from coordinator."
                            )
